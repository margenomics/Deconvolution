...
##' FARDEEP_abs_F
##'
##' Function that generates the absolute deconvolution plot with the FARDEEP method, with the fractional plot, and returns a df resulting from the deconvolution.
...
##'
##' @examples
##' FARDEEP_abs_F("Counts.HUMAN_nodup_Sal_Veh_IPs_INPUTs.txt", "Monaco-ABIS_Immune11_Microarray.txt", f, results_dir = "/bicoh/nidia/Deconv/Brain_MMartin/test", height= 8, name= "MONACO")

FARDEEP_abs_F <- function(matrix, sig.matrix, fractions, results_dir, height= 10, name= FALSE){
  require(usethis)
  require(devtools)
  require(gplots)
  require(data.table)
  ...
  for (x in 1:length(list_pos)){
    ...
    if (name==FALSE){
      ...
      file <- c("FARDEEP.abs_", title , x, "_","plot.png")
    }else{
      title <- name
      file <- c("FARDEEP.abs_", title , "_", x, "_","plot.png")
    }
