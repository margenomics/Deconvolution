...
##' EPIC_rel_NF
##'
##' Function that generates the relative deconvolution plot with the EPIC method, with the non-fractional plot, and returns a df resulting from the deconvolution.
...
##'
##' @examples
##' EPIC_rel_NF("Counts.HUMAN_nodup_Sal_Veh_IPs_INPUTs.txt", "Monaco-ABIS_Immune11_Microarray.txt", results_dir = "/bicoh/nidia/Deconv/Brain_MMartin/test", height= 8, name= "MONACO")

EPIC_rel_NF <- function(matrix, sig.matrix, results_dir, height= 10, name= FALSE){
  require(EPIC)
  require(usethis)
  require(devtools)
  require(gplots)
