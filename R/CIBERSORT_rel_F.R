...
CIBERSORT_rel_F <- function(matrix, sig.matrix, fractions, results_dir, height= 10, name= FALSE){
  require(usethis)
  require(devtools)
  require(gplots)
  require(data.table)
  ...
  # Selecció de la paleta de colors
  ...
  for (x in 1:length(list_pos)){
    ...
    if (name==FALSE){
      ...
      file <- c("CIBERSORT_", title , x, "_","plot.png")
    }else{
      title <- name
      file <- c("CIBERSORT_", title , "_", x, "_","plot.png")
    }
